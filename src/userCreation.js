...
if (data.params.firstName === 'Test'
  ...
  results.success.add('params valid');
} else {
  results.errors.add('params invalid');
}

// the api key used to make the request. not used for saving since PII key is needed
if (!data.apikey) {
  results.errors.add('apikey not available');
} else {
  results.success.add('apikey available');
}

// the session token from the login request
if (!data.session_token) {
  results.errors.add('session_token not available');
} else {
  results.success.add('session_token available');
}

// the user id from the login request
if (!data.user_id) {
  results.errors.add('user_id not available');
} else {
  results.success.add('user_id available');
}
